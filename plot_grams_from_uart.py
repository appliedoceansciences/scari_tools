...
def pgram_generator():
    ...
    for line in sys.stdin:
        ...
        if False:
            ...
            if pspl is not None:
                for id, spl_dB in enumerate(pspl.spls_dB):
                    print('bin %u: %.2f dB' % (id + pspl.iband_start, spl_dB), file=sys.stderr)
                continue

        pgram = parse_scari_pgram(line)
        if pgram is not None:
            yield pgram
...
def main():
    ...
    while True:
        ...
        intensity = np.power(10.0, np.array(packet.spl_dB) * 0.1)

        if ax and (intensity.shape[0] != X or df != packet.df or bins_per_octave != packet.bins_per_octave or dt != packet.dt):
            fig.clf()
            ax = None
        ...
        if not ax:
            ...
            df = packet.df
            dt = packet.dt
            bins_per_octave = packet.bins_per_octave
            ...
            bin_centres = [frequency_given_bin_index(x, df, bins_per_octave) for x in range(X)]
            ...
            tick_positions_Hz = [15, 30, 100, 200, 300, 400, 500, 600, 800, 1000, 1500, 2000, 3000, 4000, 5000, 6000, 8000, 10000, 15000, 20000, 30000]
